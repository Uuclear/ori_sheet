...
## 七、环刀工具集成补充（2026-06-15）

见证送样委托的 **工程见证 / 样品取样 / 规格型号 / 检测依据** 不在 `GetTestingOrdersBaseType` JSON 中，需从 `orderRow.standBy3` 指向的委托单 HTML 解析。现场检测与见证送样的「联系方式」映射规则亦不同。

详见专文：**[LIMIS见证送样字段获取说明](./LIMIS见证送样字段获取说明.md)**（含 `TG11-260350` 样例与解析格式）。

---

*文档结束 - 2026年6月8日（2026年6月15日增补第七节）*
