"""Fetch ExperimentResult3.aspx and extract API calls / field bindings."""
from __future__ import annotations

import asyncio
import base64
import json
import re

import httpx

from ring_knife.settings_store import load_settings


async def main() -> None:
    s = load_settings()
    base = s["base_url"].rstrip("/")
    page = (
        "/UI/Experiment/ExperimentResult3.aspx?sampleId=1866178&taskId=1998773"
        "&deptCode=S125&sampleName=LJ01-260364-01&taskStatusCode=2&RWindex=1"
    )
    c = httpx.AsyncClient(base_url=base, timeout=60, follow_redirects=True)
    pwd = base64.b64encode(s["password"].encode()).decode()
    r = await c.post(
        "/AjaxRequest/Index/HomeIndex.ashx",
        data={"method": "Login", "username": s["username"], "pwd": pwd},
    )
    c.cookies.set("UserId", str(r.json()["UserId"]), path="/")
    html = (await c.get(page)).text
    with open("d:/github/jsscript/experiment_page.html", "w", encoding="utf-8", errors="replace") as f:
        f.write(html)

    methods = sorted(set(re.findall(r"method\s*[:=]\s*['\"]([A-Za-z0-9_]+)['\"]", html)))
    print("methods:", methods)
    ashx = sorted(set(re.findall(r"/AjaxRequest/[A-Za-z0-9_/]+\.ashx", html)))
    print("ashx:", ashx)

    for pat in ["ReportNo", "reportNo", "报告编号", "样品名称", "sampleName", "SampleName", "回填", "Manufacturer", "TypeSpecification", "GetSample", "GetReport"]:
        if pat in html:
            print(f"found {pat}")

    headers = {
        "X-Requested-With": "XMLHttpRequest",
        "Referer": f"{base}{page}",
        "Origin": base,
    }
    for method in methods:
        for path in ashx or ["/AjaxRequest/Experiment/Experiment.ashx"]:
            data = {
                "method": method,
                "sampleId": "1866178",
                "taskId": "1998773",
                "testingOrderId": "1262331",
            }
            rr = await c.post(path, data=data, headers=headers)
            text = rr.text.strip()
            if rr.status_code == 200 and text and not text.startswith("<!") and text not in ("[]", "{}", "null") and "Object reference" not in text:
                print(f"\n{path} {method}:")
                try:
                    print(json.dumps(rr.json(), ensure_ascii=False, indent=2)[:2000])
                except Exception:
                    print(text[:300])

    await c.aclose()


if __name__ == "__main__":
    asyncio.run(main())
